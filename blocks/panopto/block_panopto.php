        ...
        try {
            if(!$panopto_data->sessiongroup_id) {
                ...
                if($course_info->Access == "Error") {
					...
					$linkurl = $course_info->CourseSettingsURL;
					$linkarray = explode('=',$linkurl);
					//echo $linkurl.' : '.$linkarray[2];
					// Fetch global settings from DB
					$panoptoserver = $CFG->block_panopto_server_name;

					if (!has_capability('moodle/course:update', $context)) {
						$linkurl = 'http://'.$panoptoserver.'/Panopto/Pages/Sessions/List.aspx#folderID=%22'.$linkarray[2].'%22';
					}
					//echo '<br />'.$linkurl;
					
//Allow all to access                    if(has_capability('moodle/course:update', $context)) {
//Edit this block                        $this->content->text .= "<div class='sectionHeader'><b>" . get_string('links', 'block_panopto') . "</b></div>
//				        						 <div class='listItem'>
...
//			        							 </div>\n";
							$this->content->text .= "<div class='sectionHeader'><b>" . get_string('links', 'block_panopto') . "</b></div>
				        						 <div class='listItem'>
				        							<a href='$linkurl' onclick='return panopto_startSSO(this)'
				        								> <img src='$CFG->wwwroot/blocks/panopto/images/panopto_logo.png' />" . get_string('course_settings', 'block_panopto') . "</a>
			        							 </div>\n";
