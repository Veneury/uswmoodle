...
if (!empty($CFG->themedir) and file_exists("$CFG->themedir/uswtwo")) {
    require_once ($CFG->themedir."/uswtwo/lib.php");
} else {
    require_once ($CFG->dirroot."/theme/uswtwo/lib.php");
}
...
//uswtwo_initialise_awesomebar($PAGE);
...
if(!empty($PAGE->theme->settings->useeditbuttons) && $PAGE->user_allowed_editing()) {
    uswtwo_initialise_editbuttons($PAGE);
    $bodyclasses[] = 'uswtwo_with_edit_buttons';
}
...
if(!empty($PAGE->theme->settings->persistentedit) && $PAGE->user_allowed_editing()) {
    ...
    $USER->editing = 1;
    $bodyclasses[] = 'uswtwo_persistent_edit';
}
...
<?php if ($hasheading || $hasnavbar) { ?>
    ...
    <?php if ($hascustommenu && empty($PAGE->theme->settings->custommenuinawesomebar)) { ?>
      <div id="custommenu" class="uswtwo-awesome-bar"><?php echo $custommenu; ?></div>
 	<?php } ?>
...
<!-- RMO *** Announcement section - a brief system announcement can be placed here to appear at the top of every page e.g. warning about service outage for maintenance etc.-->
<p style="font-size:medium; color:#be0f34; margin-bottom:0px;">Missing Modules? Click<a href="http://oln.newport.ac.uk/mod/page/view.php?id=1241" style="font-size:medium; color:#2c2c2c; text-decoration:underline; margin-bottom:0px;"> Here </a> for help</p>
<!-- RMO *******************************-->                        
...
/*    if ($requiresrespond) {
        if (!empty($CFG->themedir) and file_exists("$CFG->themedir/uswtwo")) {
	    $PAGE->requires->js($CFG->themedir.'/uswtwo/yui/respond.js');
	    } else {
	    $PAGE->requires->js($CFG->dirroot.'/theme/uswtwo/yui/respond.js');
	    }
    } */
/*    if ($usingios) { //Check if the user is using iOS and serve a JS to fix a viewport re-flow bug
        if (!empty($CFG->themedir) and file_exists("$CFG->themedir/uswtwo")) {
	    $PAGE->requires->js($CFG->themedir.'/uswtwo/yui/iOS-viewport-fix.js');
	    } else {
	    $PAGE->requires->js($CFG->dirroot.'/theme/uswtwo/yui/iOS-viewport-fix.js');
	    }
    } */?>
