...
class theme_uswtwo_core_renderer extends core_renderer {
    ...
    public function footer() {
        ...
        if (defined('MDL_PERF') || (!empty($CFG->perfdebug) and $CFG->perfdebug > 7)) {
            ...
            if (defined('MDL_PERFTOFOOT') || debugging() || $CFG->perfdebug > 7) {
                $performanceinfo = uswtwo_performance_output($perf);
            }
        }
    ...
    protected function render_custom_menu(custom_menu $menu) {
        ...
        $content = html_writer::start_tag('ul', array('class'=>'dropdown dropdown-horizontal uswtwo-custom-menu'));
        // Render each child
...
class theme_uswtwo_topsettings_renderer extends plugin_renderer_base {
        ...
        foreach ($items as $item) {
            ...
            if(substr($item->id, 0, 17)=='expandable_branch' && $item->children->count()==0) {
                ...
                if(!$subnav) {
                    ...
                    $dummypage = new uswtwo_dummy_page();
                    $dummypage->set_context(get_context_instance(CONTEXT_SYSTEM));
                    $dummypage->set_url($PAGE->url);
                    $subnav = new uswtwo_expand_navigation($dummypage, $item->type, $item->key);
                } else {
