...
class theme_newu_core_renderer extends core_renderer {
    ...
    public function footer() {
        ...
        if (defined('MDL_PERF') || (!empty($CFG->perfdebug) and $CFG->perfdebug > 7)) {
            ...
            if (defined('MDL_PERFTOFOOT') || debugging() || $CFG->perfdebug > 7) {
                $performanceinfo = newu_performance_output($perf);
            }
        }
	...
	global $USER;
        ...
        if (isloggedin() && !isguestuser() && $mycourses = enrol_get_my_courses(NULL, 'visible DESC, fullname ASC')) {
            $mycoursesmenu = $menu->add(/*get_string('mycourses')*/"Moodle2.5 Courses (2013-14)", new moodle_url('https://my.newport.ac.uk/moodle25/my'), get_string('mycourses'), 3);// lower numbers = higher priority e.g. move this item to the left on the Custom Menu
            foreach ($mycourses as $mycourse) {
                $mycoursesmenu->add($mycourse->shortname, new moodle_url('/course/view.php', array('id' => $mycourse->id)), $mycourse->fullname);
            }
        }
        
        //Add other standard menu items (structure taken from my.newport menu items).
        //News - staff only.
        if ($usertype == "staff") {
            $branchlabel = "News"; //get_string('menunews');
            $branchurl = new moodle_url('https://my.newport.ac.uk/uniportal/news/index.aspx');
            $branchsort = '11';
            $branch = $menu->add($branchlabel,$branchurl,$branchlabel,$branchsort);
        }
        //MyDocs - All.
        $branchlabel = "MyDocs"; //get_string('menumydocs');
        $branchurl = new moodle_url('https://my.newport.ac.uk/uniportal/mydocs/mydocs.aspx');
        $branchsort = '12';
        $branch = $menu->add($branchlabel,$branchurl,$branchlabel,$branchsort);
        //PhoneBook - Staff.
        if ($usertype == "staff") {
            $branchlabel = "PhoneBook"; //get_string('menuphonebook');
            $branchurl = new moodle_url('https://my.newport.ac.uk/uniportal/phone/index.aspx');
            $branchsort = '13';
            $branch = $menu->add($branchlabel,$branchurl,$branchlabel,$branchsort);
        }
        //UniSite - All
        $branchlabel = "UniSite"; //get_string('menuunisite');
        $branchurl = new moodle_url('http://www.southwales.ac.uk/');
        $branchsort = '14';
        $branch = $menu->add($branchlabel,$branchurl,$branchlabel,$branchsort);
        //Email - Staff/Students separate
        if ($usertype == "staff") {
            $branchlabel = "Email"; //get_string('menuemail');
            $branchurl = new moodle_url('https://my.newport.ac.uk/uniportal/content/displaypage.aspx?object_id=3474&object_type=page&type=3');
            $branchsort = '15';
        } else {
            $branchlabel = "Email"; //get_string('menuemail');
            $branchurl = new moodle_url('https://my.newport.ac.uk/uniportal/email.aspx');
            $branchsort = '15';
        }
        $branch = $menu->add($branchlabel,$branchurl,$branchlabel,$branchsort);
        //Events - Staff
        if ($usertype == "staff") {
            $branchlabel = "Events"; //get_string('menuevents');
            $branchurl = new moodle_url('https://my.newport.ac.uk/uniportal/phone/index.aspx');
            $branchsort = '16';
            $branch = $menu->add($branchlabel,$branchurl,$branchlabel,$branchsort);
                $branch->add('Calendar',new moodle_url('https://my.newport.ac.uk/uniportal/calendar.aspx'),'Calendar');
                $branch->add('Event Logging form',new moodle_url('https://my.newport.ac.uk/uniportal/content/displayPage.aspx?object_id=2098&object_type=page&type=3'),'Event logging form');
        }
        //StudentLife - Students
        if ($usertype == "student") {
            $branchlabel = "StudentLife"; //get_string('menustudentlife');
            $branchurl = new moodle_url('https://my.newport.ac.uk/uniportal/content/displaypage.aspx?object_id=159&object_type=page&type=6');
            $branchsort = '17';
            $branch = $menu->add($branchlabel,$branchurl,$branchlabel,$branchsort);
        }
        //Library - All
        $branchlabel = "Library"; //get_string('menulibrary');
        $branchurl = new moodle_url('http://studentlibrary.southwales.ac.uk/');
        $branchsort = '18';
        $branch = $menu->add($branchlabel,$branchurl,$branchlabel,$branchsort);
        //MyHR - Staff
        if ($usertype == "staff") {
            $branchlabel = "MyHR"; //get_string('menumyhr');
            $branchurl = new moodle_url('https://myhr.newport.ac.uk/live/Portal/');
            $branchsort = '19';
            $branch = $menu->add($branchlabel,$branchurl,$branchlabel,$branchsort);
        }
        //SU - Students
        if ($usertype == "student") {
            $branchlabel = "SU"; //get_string('menusu');
            $branchurl = new moodle_url('http://www.newportunion.com/');
            $branchsort = '20';
            $branch = $menu->add($branchlabel,$branchurl,$branchlabel,$branchsort);
        }
		//Learning Review - Students
 //       if ($usertype == "student") {
            $branchlabel = "Learning Environment Survey"; //get_string('menustudentlife');
            $branchurl = new moodle_url('https://formative.glam.ac.uk/q4/open.dll?session=2972247400760388&Group=LER&Name=Student');
            $branchsort = '22';
			$branchtitle = "Click this link to complete the survey and comment about the Learning Environment";
            $branch = $menu->add($branchlabel,$branchurl,$branchtitle,$branchsort);
 //       }

        
        // Initialise this custom menu
        $content = html_writer::start_tag('ul', array('class'=>'dropdown dropdown-horizontal newu-custom-menu'));
        // Render each child
...
class theme_newu_topsettings_renderer extends plugin_renderer_base {
        ...
        foreach ($items as $item) {
            ...
            if(substr($item->id, 0, 17)=='expandable_branch' && $item->children->count()==0) {
                ...
                if(!$subnav) {
                    ...
                    $dummypage = new newu_dummy_page();
                    $dummypage->set_context(get_context_instance(CONTEXT_SYSTEM));
                    $dummypage->set_url($PAGE->url);
                    $subnav = new newu_expand_navigation($dummypage, $item->type, $item->key);
                } else {
