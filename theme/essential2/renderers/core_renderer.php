...
class theme_essential2_core_renderer extends core_renderer {
...
/** @var custom_menu_item language The language menu if created */
    ...
    public function navbar() {
        $breadcrumbs = '';
        $breadcrumbstyle = theme_essential2_get_setting('breadcrumbstyle');
        if ($breadcrumbstyle) {
            $index = 1;
    ...
    public function footer() {
        ...
        if (defined('MDL_PERF') || (!empty($CFG->perfdebug) and $CFG->perfdebug > 7)) {
            ...
            if (defined('MDL_PERFTOFOOT') || debugging() || $CFG->perfdebug > 7) {
                $performanceinfo = theme_essential2_performance_output($perf, theme_essential2_get_setting('perfinfo'));
            }
        }
        ...
        if(theme_essential2_get_setting('persistentedit') && property_exists($USER, 'editing') && $USER->editing && !$this->really_editing) {
            $USER->editing = false;
        }
    ...
    public function custom_menu_courses() {
        ...
        $hasdisplaymycourses = theme_essential2_get_setting('displaymycourses');
        if (isloggedin() && !isguestuser() && $hasdisplaymycourses) {
            $mycoursetitle = theme_essential2_get_setting('mycoursetitle');
    ...
    public function custom_menu_themecolours() {
        ...
        if (!isguestuser()) {
            ...
            foreach (range(1, 3) as $alternativethemenumber) {
                if (theme_essential2_get_setting('enablealternativethemecolors' . $alternativethemenumber)) {
                    $alternativethemes[] = $alternativethemenumber;
                }
            }
            if (!empty($alternativethemes)) {
                $branchtitle = get_string('themecolors', 'theme_essential2');
                $branchlabel = '<i class="fa fa-th-large"></i>' . $branchtitle;
                $branchurl   = new moodle_url('#');
                $branchsort  = 300;
                $branch = $colourmenu->add($branchlabel, $branchurl, $branchtitle, $branchsort);
                
                $defaultthemecolorslabel = get_string('defaultcolors', 'theme_essential2');
                $branch->add('<i class="fa fa-square colours-default"></i>' . $defaultthemecolorslabel,
                        new moodle_url($this->page->url, array('essential2colours' => 'default')), $defaultthemecolorslabel);
                foreach ($alternativethemes as $alternativethemenumber) {
                    if (theme_essential2_get_setting('alternativethemename' . $alternativethemenumber)) {
                        $alternativethemeslabel = theme_essential2_get_setting('alternativethemename' . $alternativethemenumber);
                    } else {
                        $alternativethemeslabel = get_string('alternativecolors', 'theme_essential2', $alternativethemenumber);
                    }
                    $branch->add('<i class="fa fa-square colours-alternative' .  $alternativethemenumber . '"></i>' . $alternativethemeslabel,
                            new moodle_url($this->page->url, array('essential2colours' => 'alternative' . $alternativethemenumber)), $alternativethemeslabel);
                }
            }
        }
    ...
    protected function get_time_difference($created_time) {
        ...
        if(intval($time_difference/$years) > 1) {
            ...
            return get_string('ago', 'core_message', get_string('few', 'theme_essential2').get_string('seconds'));
        }
    }
    ...
    public function custom_menu_user() {
        ...
        if (isloggedin() && !isguestuser()) {
            ...
            if (\core\session\manager::is_loggedinas()) {
                ...
                $branchlabel = '<em><i class="fa fa-key"></i>'.fullname($realuser, true).get_string('loggedinas', 'theme_essential2').fullname($USER, true).'</em>';
            ...
            if (is_mnet_remote_user($USER) and $idprovider = $DB->get_record('mnet_host', array('id'=>$USER->mnethostid))) {
                $branchlabel = '<em><i class="fa fa-users"></i>'.get_string('loggedinfrom' , 'theme_essential2').$idprovider->name.'</em>';
            ...
            if ($course->id == 1) {
                    ...
                    foreach ($hascourses as $hascourse) {
                        if ($hascourse->visible) {
                            ...
                            if (has_capability ('gradereport/user:view', $context)) {
                                $branchlabel = '<em><i class="fa fa-list-alt"></i>'.get_string('mygrades', 'theme_essential2').'</em>';
                ...
                if (has_capability ('gradereport/user:view', $context)) {
                    $branchlabel = '<em><i class="fa fa-list-alt"></i>'.get_string('coursegrades', 'theme_essential2').'</em>';
            ...
            if (theme_essential2_get_setting('helplinktype')) {
                $branchlabel = '<em><i class="fa fa-question-circle"></i>'.get_string('help').'</em>';
                $branchurl   = new moodle_url('#');
                $target      = '';
                switch(theme_essential2_get_setting('helplinktype')) {
                    case 1:
                    if (filter_var(theme_essential2_get_setting('helplink'), FILTER_VALIDATE_EMAIL)) {
                        $branchurl = 'mailto:'.theme_essential2_get_setting('helplink').'?cc='.$USER->email;
                    } else {
                        $branchlabel = '<em><i class="fa fa-exclamation-triangle red"></i>'.get_string('invalidemail').'</em>';
                    }
                    break;
                    case 2:
                    if(filter_var(theme_essential2_get_setting('helplink'), FILTER_VALIDATE_URL,FILTER_FLAG_SCHEME_REQUIRED)) {
                        $branchurl = theme_essential2_get_setting('helplink');
                        $target    = '_blank';
                    } else {
    ...
    */
    ...
    public function render_social_network($socialnetwork) {
        if (theme_essential2_get_setting($socialnetwork)) {
            ...
            $socialhtml .= html_writer::start_tag('button', array('type' => "button",
                                                                  ...
                                                                  'onclick' => "window.open('".theme_essential2_get_setting($socialnetwork)."')",
                                                                  'title' => get_string($socialnetwork, 'theme_essential2'),
                                                                  ));
