YUI.add('moodle-theme_newu-editbuttons', function(Y) {
...
EditButtons.prototype = {
    ...
    initializer : function(config) {
        ...
        if(!this.editbutton) {
            ...
            this.editbutton.addClass('newu-editbutton');
        }
        ...
        try {
            ...
            M.core_dock.getPanel().on('dockpanel:beforehide', function(e) {
                var icons = this.one('.dockeditempanel_hd .commands');
                var closeicon = this.one('.newu-editbutton-wrap .hidepanelicon');
                if(closeicon) {
    ...
    wrapButton : function(icons, button) {
        try{
            icons.wrap('<div class="newu-editbutton-wrap"></div>').insert(button, icons);
        } catch(x) {
            // Fallback for old versions of YUI without Node.wrap() (MDL20)
            var wrapper = Y.DOM.create('<div class="newu-editbutton-wrap"></div>');
    ...
    processIcons : function(icons) {
        ...
        thisbutton.on('click', function(e, button) {
            ...
            if(mod) mod.toggleClass('newu-editbutton-active-module');
        }, this, thisbutton);
...
M.theme_newu = M.theme_newu || {};
// Initialisation function for the colour switcher
M.theme_newu.initEditButtons = function(cfg) {
